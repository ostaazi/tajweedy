...
// Types for segments
type WordSegment = [number, number, number]; // [word_position, start_ms, end_ms]
type VerseTimestamp = {
  verse_key: string;
...
export default function RecitationPage() {
  ...
  const [availableAyahs, setAvailableAyahs] = useState<any[]>([]);
  ...
  const fetchVerse = async () => {
    ...
    try {
      ...
      if (verseData.status === 'OK') {
        ...
        // Fetch words with position and type filtering
        ...
        const allWords = wordsData?.verse?.words || [];
        
        // Filter only actual words (exclude end markers)
        const displayWords = allWords.filter((w: any) => w.type === 'word');
        
        // Build position to display index map
        const posToIdx = new Map<number, number>();
        displayWords.forEach((w: any, i: number) => {
          if (typeof w.position === 'number') {
            posToIdx.set(w.position, i);
          }
        });
  ...
  // Auto-highlighting with segments - FIXED VERSION
  useEffect(() => {
    ...
    const attach = () => {
      ...
      const onTimeUpdate = () => {
        ...
        if (k !== -1) {
          const pos1 = wordSegments[k][0]; // position (1-based)
      ...
      attached = true;
      (window as any).__onTimeUpdate = onTimeUpdate;
    };

    // Attach after metadata is loaded
    ...
    return () => {
      const onTimeUpdate = (window as any).__onTimeUpdate;
      if (onTimeUpdate) {
        audioEl.removeEventListener('timeupdate', onTimeUpdate);
        audioEl.removeEventListener('seeked', onTimeUpdate);
        (window as any).__onTimeUpdate = null;
      }
                ...
                {surahs.map(surah => (
                  <option key={surah.id} value={surah.id}>
                    {surah.subtext 
                      ...
                      : surah.id > 0 ? `${surah.id}. ${surah.name}` : surah.name}
                  </option>
                ))}
              </select>
            ...
            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 mb-4">
              <p className="text-sm text-gray-600 mb-2 text-center">
                ...
                {wordSegments.length > 0 && (
                  <span className="text-green-600 font-bold mr-2">✓ مزامنة دقيقة</span>
                )}
              </p>
              {verse?.audio ? (
                <audio 
            ...
            <div className="bg-yellow-50 border-r-4 border-yellow-400 p-4 rounded-lg">
              <p className="text-sm text-gray-700">
                💡 <strong>تلميح:</strong> {wordSegments.length > 0 
                  ...
                  : 'انقر على أي كلمة لتمييزها. يتم تحميل المزامنة التلقائية...'}
              </p>
            </div>
          </div>
        )}
