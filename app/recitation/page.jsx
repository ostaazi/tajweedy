...
/**
 * ملاحظة: chapterReciterId افتراضي لمشاري العفاسي والشاطري فقط
 * والباقي fallback على العفاسي في المزامنة، مع بقاء الاسم كما هو.
 */
const RECITERS = [
  {
    id: 0,
...
export default function RecitationPage() {
  ...
  const [currentVerseWindow, setCurrentVerseWindow] = useState(null);
  const [wordTimings, setWordTimings] = useState([]);

  const audioRef = useRef(null);

  /* ========= تحميل السور + الآية الأولى عند فتح الصفحة ========= */
  useEffect(() => {
    fetchSurahs();
    fetchVerse(); // <—— هذا هو التعديل المهم
  ...
  const loadChapterAudio = async (chapterReciterId, surahNum) => {
    ...
    if (!resp.ok) throw new Error('تعذر تحميل ملف السورة الصوتي مع التوقيتات');
    ...
    timestampsArray.forEach((ts) => {
      if (ts.verse_key) tsMap[ts.verse_key] = ts;
    });
  ...
  const prepareVerseTiming = (surahNum, ayahNum, tsMap) => {
    ...
    const segs = ts.segments.map(([wordIndex, startMs, endMs]) => ({
      wordIndex: (wordIndex || 1) - 1,
      start: startMs,
      end: endMs,
    }));
    setWordTimings(segs);
  ...
  const fetchVerse = async () => {
    ...
    try {
      ...
      setVerse((prev) =>
        prev
          ? { ...prev, audio: audioFile.audio_url, reciter: reciterData.name }
          : prev
      );
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-6 md:p-8 mb-6">
              ...
              {/* اختيارات القارئ + البداية والنهاية */}
              ...
              <div className="bg-yellow-50 border-r-4 border-yellow-400 p-4 rounded-lg mt-2">
                <p className="text-sm text-gray-700 flex items-start gap-2">
                  ...
                  <span>
                    ...
                    الآية والصوت الصحيح مع مزامنة الكلمات قدر الإمكان.
                  </span>
                </p>
              </div>
            </div>
