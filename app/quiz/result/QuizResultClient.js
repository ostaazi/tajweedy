// app/quiz/result/QuizResultClient.js
'use client';
...
/* ======================= QR Code Component ======================= */
function QrCode({ value, size = 300, className = '' }) {
  ...
  useEffect(() => {
    ...
    const services = [
      ...
      `https://chart.googleapis.com/chart?cht=qr&chs=${size}x${size}&chl=${encodeURIComponent(
        value
      )}`,
    ];
      ...
      {src && !error ? (
        <>
          <img
            ...
            className="mx-auto rounded-2xl border-4 border-green-300 shadow"
          />
          <button
            type="button"
            onClick={download}
            className="mt-3 px-4 py-2 rounded-xl bg-blue-600 text-white font-bold no-print"
          >
            تحميل QR 📥
          </button>
        </>
...
export default function QuizResultClient() {
  ...
  // URL لقيمة الـ QR (يعتمد على attemptId)
  const qrValue = useMemo(() => {
    if (!attemptId) return '';
    if (typeof window === 'undefined') return '';
    return `${window.location.origin}/quiz/result?id=${attemptId}`;
  }, [attemptId]);

  // احصاءات محسوبة عند تغيّر attempt
  const aggregates = useMemo(() => {
    ...
    attempt.responses.forEach(r => {
      ...
      if (!sMap[sKey]) {
        sMap[sKey] = { section: getArabicSectionName(sKey), subs: {}, right: 0, wrong: 0, total: 0 };
      }
      sMap[sKey].total++;
      ...
      if (subKey) {
        if (!sMap[sKey].subs[subKey]) {
          sMap[sKey].subs[subKey] = { subsection: getArabicSubsectionName(subKey), right: 0, wrong: 0, total: 0 };
        }
    ...
    const sArr = Object.values(sMap)
      .map((s) => ({
        ...s,
        subs: Object.values(s.subs || {})
          .map((sub) => ({
            ...sub,
        ...
        * { font-family: 'Cairo', sans-serif !important; }
        ...
        @media print {
          ...
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          ...
          #result-content { position: absolute; left: 0; top: 0; width: 100%; background: white; padding: 20px; }
        ...
        <div id="result-content" className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 text-center">
            ...
            <div className="flex justify-center mb-4">
              <svg width="150" height="150" viewBox="0 0 150 150" aria-label="final percentage">
          ...
          {qrValue && (
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 text-center page-break-after">
              ...
              <QrCode value={qrValue} size={300} className="mx-auto mb-4" />
