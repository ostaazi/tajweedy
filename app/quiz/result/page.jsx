...
// تحويل التاريخ إلى أرقام إنجليزية للبطاقة
function getEnglishDate(dateLike) {
  const d = (dateLike instanceof Date) ? dateLike : new Date(dateLike || Date.now());
...
export default function QuizReportPage() {
  ...
  // دالة الرسم البياني (مطابقة للنموذج: أعمدة متجاورة، دليل رأسي)
  const BarChart = ({ subs }) => {
    ...
    return (
      <div className="mt-6 w-full">
        <svg width="100%" height={chartHeight + 100} viewBox={`0 0 ${chartWidth} ${chartHeight + 100}`} className="bg-gray-50 rounded-lg p-4 block mx-auto">
          {/* المحاور */}
          ...
          {/* التسميات على X (أسماء عربية دوّارة) */}
          {subs.map((sub, idx) => (
            <g key={idx}>
              <text 
                x={50 + idx * groupSpacing + groupWidth / 2} 
                y={chartHeight + 10} 
                textAnchor="middle" 
                fontSize="12" 
                fill="#374151"
                transform={`rotate(-45 ${50 + idx * groupSpacing + groupWidth / 2} ${chartHeight + 10})`}
                dir="rtl"
              >
                {sub.subsection}
              </text>
          ...
          {subs.map((sub, idx) => {
            ...
            return (
              <g key={idx}>
                ...
                />
                <text
                  x={xBase + barWidth + barSpacing + barWidth / 2}
          ...
          {/* الدليل (Legend) - رأسي كما في النموذج */}
          <g transform={`translate(${chartWidth - 150}, ${chartHeight + 30})`}>
            {/* سطر أخضر */}
            <rect x="0" y="0" width="15" height="15" fill={COLORS.correct} rx="2" />
            <text x="20" y="12" fontSize="12" fill="#374151">صحيح</text>
            {/* سطر أحمر أسفل */}
            <rect x="0" y="20" width="15" height="15" fill={COLORS.wrong} rx="2" />
            <text x="20" y="32" fontSize="12" fill="#374151">خطأ</text>
          </g>
        </svg>
      </div>
    );
  ...
  const examDateArabic = formatDateArabic(attempt.date || Date.now());
  const examDateEnglish = toEnglishDigits(getEnglishDate(attempt.date || Date.now()));
    ...
    <>
      <style jsx global>{`
        ...
        * {
        ...
        /* Watermark - مصغّرة ومحاذاة للوسط */
        .watermark-bg {
          position: fixed;
          top: 0;
          left: 0;
          ...
          background-size: 60% auto; /* تصغير الحجم */
          background-repeat: no-repeat;
          background-position: center center; /* محاذاة أفقي ورأسي */
          opacity: 0.1;
          z-index: 0;
          pointer-events: none;
        }
        ...
        @media print {
          ...
          * {
          ...
          /* Watermark for print - مصغّرة ومحاذاة */
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 p-4 md:p-8 relative z-10" dir="rtl">
        <div id="report-content" className="max-w-6xl mx-auto">
          ...
          {/* Header - شعار فوق منتصف النص، نص يمين */}
          <div className="text-center mb-6">
            ...
            <p className="text-base font-bold text-primary text-right mx-auto max-w-md">التجويد الذكي</p>
          </div>

          {/* بطاقة الترويسة - أعلى يسار، RTL داخلي */}
          <div className="bg-white rounded-3xl shadow-lg p-4 mb-6 w-64 absolute top-4 left-4" dir="rtl">
            <div className="text-right">
              ...
              <p className="text-base text-gray-600">
                <strong>تاريخ الاختبار:</strong> <span className="dir=ltr ml-2">{examDateEnglish}</span>
              </p>
            </div>
          </div>

          {/* اسم المتدرب - مركز */}
          <div className="bg-white rounded-3xl shadow-lg p-6 mb-6 text-center clear-both">
